# sweep launcher for split notmnist vcl w/ active coreset selection
...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        experiment_name = (
            f"active_split_notmnist_"
            f"lr{config.get('lr', 0.001)}_"
            f"lambda{config.get('lambda_mix', 0.5)}_"
            f"coreset{config.get('coreset_size', 200)}_"
            f"kc{config.get('use_kcenter', True)}_"
            f"ep{config.get('epochs_per_task', 120)}_"
        ...
        print(f"  Coreset size: {config.get('coreset_size', 200)}")
        print(f"  Use K-center: {config.get('use_kcenter', True)}")
        print(f"  Tasks: {config.get('num_tasks', 5)}")
        print(f"  Epochs per task: {config.get('epochs_per_task', 120)}")
...
def main():
    ...
    sweep_config_path=os.path.join(root_dir, "z_active_coreset/active_coreset_sp_notmnist.yaml")
    output_dir = os.path.join(root_dir, "Active_Coreset_Split_notMNIST") 
    project_name = "vcl-active-coreset-split-notmnist"
    ...
    wandb.agent(sweep_id, function=sweep_fn, count=27)
