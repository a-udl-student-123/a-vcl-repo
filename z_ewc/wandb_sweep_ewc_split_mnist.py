...
from z_ewc.experiment_runner_ewc import run_ewc_experiment
from z_utils.experiment_utils import is_experiment_completed

OUTPUT_DIR = "EWC_SplitMNIST"
FORCE_RERUN = False
COUNT = 30  # num experiments to run
PROJECT_NAME = "EWC-split-mnist"
...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        experiment_name = (
            f"ewc_split_mnist_"
            f"h{config.get('hidden_size', 256)}_"
            f"ep{config.get('epochs_per_task', 15)}_"
            f"lr{config.get('lr', 0.01)}_" 
            f"lambda{config.get('ewc_lambda', 100.0)}_"
        ...
        print(f"Running Split MNIST experiment: {experiment_name}")
        print(f"  Hidden size: {config.get('hidden_size', 256)}")
        print(f"  Learning rate: {config.get('lr', 0.01)}")
        print(f"  EWC lambda: {config.get('ewc_lambda', 100.0)}")
        print(f"  Fisher samples: {config.get('n_train_samples', 600)}")
        print(f"  Epochs per task: {config.get('epochs_per_task', 15)}")  
...
def main():
    ...
    sweep_config_path = os.path.join(root_dir, "z_ewc/sweep_ewc_split_mnist.yaml")
    output_dir = os.path.join(root_dir, OUTPUT_DIR)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    print(f"Starting EWC Split MNIST sweep with {COUNT} iterations")
