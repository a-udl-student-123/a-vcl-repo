...
OUTPUT_DIR = "EWC_SplitNotMNIST"
FORCE_RERUN = False
COUNT = 30  
PROJECT_NAME = "EWC-split-notmnist"
...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        print(f"  Fisher samples: {config.get('n_train_samples', 2000)}")
        ...
        run_ewc_experiment(
            ...
            experiment_dir, 
            device=device,
            use_wandb=True  # already in wandb run
        )
...
def main():
    ...
    sweep_config_path = os.path.join(root_dir, "z_ewc/sweep_ewc_split_notmnist.yaml")
    output_dir = os.path.join(root_dir, OUTPUT_DIR)
    device = "cuda" if torch.cuda.is_available() else "cpu"

    print(f"Starting EWC sweep with {COUNT} iterations")
