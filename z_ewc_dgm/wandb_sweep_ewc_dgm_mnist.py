...
def create_sweep_function(base_dir, device, force_rerun=False):
    def sweep_iteration():
        ...
        print(f"done - saved to {experiment_dir}")
    
    return sweep_iteration

def main():
    root_dir = os.path.dirname(os.path.abspath(__file__))
    sweep_config_path = os.path.join(root_dir, "sweep_dgm_ewc_mnist.yaml")
    output_dir = os.path.join(root_dir, "..", "EWC_DGM_MNIST")
    ...
    sweep_id = wandb.sweep(sweep_config, project="EWC-dgm-mnist")
