...
OUTPUT_DIR = "LP_SplitMNIST"
FORCE_RERUN = False
COUNT = 48
PROJECT_NAME = "LP-split-mnist"
...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        print(f"experiment completed. results in {experiment_dir}")
    
    return sweep_iteration

def main():
    root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    sweep_config_path = os.path.join(root_dir, "z_lp/sweep_lp_split_mnist.yaml")
    output_dir = os.path.join(root_dir, OUTPUT_DIR)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    print(f"starting LP split mnist sweep with {COUNT} iterations")
