# training utils for LP on deep generative models (mnist/notmnist)
# mostly same as SI version but with different loss/curvature handling
...
from .lp_vae import LP_DGM_VAE

def _train_epoch(model, loader, optimiser, lp_lambda, device):
    model.train().to(device)
    data_size = len(loader.dataset)
    totals = dict(total_loss=0., recon_loss=0., kl_latent=0., lp_penalty=0.)
    seen = 0

    for xb, _ in tqdm(loader, leave=False, desc="batches"):
        xb = xb.to(device)
        optimiser.zero_grad(set_to_none=True)
        loss, m = model.compute_loss(xb, lp_lambda, data_size)
        loss.backward()
        ...
        bs = xb.size(0)
        seen += bs
        for k in ("total_loss", "recon_loss", "kl_latent", "lp_penalty"):
    ...
    display_stats = {
        ...
        "lp": totals["lp_penalty"] / seen
    }
    
    return display_stats


def _train_task(model, loader, task_idx, epochs, lr, lp_lambda, device,
    ...
    for ep in ep_bar:
        stats = _train_epoch(model, loader, opt, lp_lambda, device)
...
def _create_montage(model, out_dir: Path, task_idx: int, montage_idx: int = 1):
...
def _visualise_reconstructions(model, loader, out_dir: Path,
                               task_idx: int, k: int = 5):
    out_dir.mkdir(exist_ok=True, parents=True)
    model.eval()
    xb, _ = next(iter(loader))
    xb = xb[:k].to(next(model.parameters()).device)
    with torch.no_grad():
        # use sample() for diversity instead of reconstruct()
...
def _print_eval(task_idx, results):
    ...
    for i, r in enumerate(results[:-1]):
        output = f"  Task {i+1} | recon {r['recon_error']:.4f} | LL {r['log_likelihood']:.4f}"
        if has_classifier:
            output += f" | CU {r.get('classifier_uncertainty', 0.0):.4f}"
        print(output)
    
    avg = results[-1]
    avg_output = f"  Avg  | recon {avg.get('average_recon_error', 0.0):.4f} | LL {avg.get('average_log_likelihood', 0.0):.4f}"
    
    if has_classifier:
        avg_output += f" | CU {avg.get('average_classifier_uncertainty', 0.0):.4f}"
    elif 'average_classifier_uncertainty' in avg:
        avg_output += f" | CU {avg['average_classifier_uncertainty']:.4f}"
...
def train_lp_vae(
    ...
    momentum=0.9,
    lp_lambda=10.0,
    n_hessian_samples=600,
    device="cuda",
    output_dir="experiments/lp_vae",
    classifier=None,
    ...
    model = LP_DGM_VAE(input_size, hidden_size, latent_size, num_tasks).to(device)

    test_loaders, task_metrics = [], []
    for t in range(num_tasks):
        print(f"\n──── Task {t+1}/{num_tasks} ────")
        train_loader = train_loader_factories[t]()

        _train_task(model, train_loader, t, epochs, lr, lp_lambda, device,
                    optimiser_type, momentum, record_metric_fn)

        # get hessian and save checkpoint
        print("  estimating Hessian ...")
        h = model.estimate_hessian(train_loader, device, n_samples=n_hessian_samples)
        model.register_lp_task(h)
        torch.save(model.state_dict(), out / "checkpoints" / f"model_t{t+1}.pt")

        # generate samples and reconstructions
        montage_paths = []
        for m in range(5):
            montage_paths.append(_create_montage(model, out / "samples", t, m+1))
        
        try:
            if wandb.run is not None:
                global_step = (t + 1) * epochs
                wandb.log({
                    f"samples/task{t+1}_montage": wandb.Image(str(montage_paths[0]))
                }, step=global_step)
        except ImportError:
            # wandb not available, skip logging
            pass
        ...
        _visualise_reconstructions(model, test_loaders[t], out / "samples", t)
    ...
    print(f"\nTraining finished in {total/60:.1f} min  |  checkpoints -> {out/'checkpoints'}")
