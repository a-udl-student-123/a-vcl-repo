...
from z_lp_dgm.experiment_runner_lp_dgm import run_lp_vae_experiment
...
def create_sweep_function(base_dir, device, force_rerun=False):
    def sweep_iteration():
        ...
        experiment_name = (
            ...
            f"lambda{config.get('lp_lambda', 1.0)}_"
        ...
        print(f"  LP lambda: {config.get('lp_lambda', 1.0)}")
...
def main():
    ...
    sweep_config_path = os.path.join(root_dir, "sweep_dgm_lp_notmnist.yaml")
    output_dir = os.path.join(root_dir, "..", "LP_DGM_notMNIST") 
    ...
    sweep_id = wandb.sweep(sweep_config, project="LP-dgm-notmnist")
    ...
    wandb.agent(sweep_id, function=sweep_fn, count=3)
