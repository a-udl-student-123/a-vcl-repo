...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        experiment_name = (
            ...
            f"std{config.get('init_std', 0.05)}_"
            ...
            f"diffperm{config.get('different_perm_init', True)}_"
        ...
        print(f"  Initial std: {config.get('init_std', 0.05)}")
        print(f"  Coreset size: {config.get('coreset_size', 200)}")
        print(f"  Use K-center: {config.get('use_kcenter', True)}")
        print(f"  Different perm init: {config.get('different_perm_init', True)}")
...
def main():
    ...
    sweep_config_path = os.path.join(root_dir, "z_sweep_configs/perm_mnist/coreset_vcl1_sweep.yaml")
    output_dir = os.path.join(root_dir, "Permuted_MNIST/CoresetVCL")
    ...
    wandb.agent(sweep_id, function=sweep_fn, count=6)
