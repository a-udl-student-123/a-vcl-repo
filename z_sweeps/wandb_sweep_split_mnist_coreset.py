...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        print(f"  Coreset size: {config.get('coreset_size', 0)}")
        print(f"  Use K-center: {config.get('use_kcenter', False)}")
        ...
        print(f"  Epochs: {config.get('epochs', 120)}")
        print(f"  Single batch: {config.get('single_batch', True)}")
...
def main():
    ...
    sweep_config_path = os.path.join(root_dir, "z_sweep_configs/sp_mnist/coreset_vcl_sweep.yaml")
    output_dir = os.path.join(root_dir, "Split_MNIST/CoresetVCL")
    project_name = "vcl-split-mnist-fixed"
    ...
    wandb.agent(sweep_id, function=sweep_fn, count=40)
