...
def create_sweep_function(base_dir, device, force_rerun=False):
    ...
    def sweep_iteration():
        ...
        run_multi_head_si_experiment_with_wandb(
            config, 
            experiment_dir,
            project_name="SI-split-notmnist-sweep",
            device=device
        )
...
def main():
    ...
    sweep_config_path = os.path.join(root_dir, "..", "z_synaptic/sweep_split_notmnist.yaml")
    output_dir = os.path.join(root_dir, "..", "Split_notMNIST/SI")
    project_name = "SI-split-notmnist-sweep-fixed-si"
    ...
    wandb.agent(sweep_id, function=sweep_fn, count=30)
